...
function shipping_price($originIso,$_pays,$_poids,$activerPromoLivraison,$totalHtFinal,$livraisonComprise) {
    ...
    $Orderquery = mysql_query("SELECT products_ean, products_garantie_".$lang.", products_download, products_deee, products_name_".$lang.", products_desc_".$lang.", products_price, products_image, products_ref, products_id, categories_id, products_qt, fabricant_id, products_weight
                               FROM products
    ...
    if($OrderqueryNum > 0) {
            ...
            while ($arrSelect = mysql_fetch_array($Orderquery)) {
                    ...
                    if($arrSelect['products_qt']>0 ) $delivery = '3-5 jours';
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="oui" AND $actRes=="oui") $delivery = '7-10 jours';
                    if($arrSelect['products_download']=="yes") $delivery = '1 heures';
                    
                    $d3e = ($arrSelect['products_deee']>0)? "-2" : 0;
                    
                    
                    $granti = (!empty($arrSelect['products_garantie_'.$lang]))? strip_tags($arrSelect['products_garantie_'.$lang]) : 0;
                    
                    $ean = strip_tags($arrSelect['products_ean']);
                    
                    $arr[] = $marque."\t".$category."\t".$offerId."\t".$title."\t".$desc."\t".$prix."\t".$prix_barre."\t".$url."\t".$image."\t".strtoupper(substr($devise,0,3))."\t".$availability."\t".$sf."\t".$delivery."\t".$d3e."\t".$granti."\t".$ean;
            }

                    
                    header("Content-Type: application/csv-tab-delimited-table");
                    header("Content-disposition:attachment; filename=leguide_".$country.".txt");
