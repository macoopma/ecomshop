...
// function KELKOO
//----------------
function build_shopping_file($lang) {
GLOBAL $_POST;
    ...
    if(isset($_POST['categShopping']) and count($_POST['categShopping']) > 0) {
        $catToExport = "";
        $numm = count($_POST['categShopping']);
    	    for($i=0; $i<$numm; $i++) {
    	        if($i == 0) {$catToExport .= " categories_id = '".$_POST['categShopping'][$i]."' ";}
    	    	  else {$catToExport .= " OR categories_id = '".$_POST['categShopping'][$i]."' ";}
    		}
...
function shipping_price($originIso,$_pays,$_poids,$activerPromoLivraison,$totalHtFinal,$livraisonComprise) {
    ...
    $Orderquery = mysql_query("SELECT products_garantie_".$lang.", products_ean, products_download, products_deee, products_name_".$lang.", products_desc_".$lang.", products_price, products_image, products_ref, products_id, categories_id, products_qt, fabricant_id, products_weight
                               FROM products
    ...
    if($OrderqueryNum > 0) {
            ...
            while ($arrSelect = mysql_fetch_array($Orderquery)) {
                    ...
                    $marqueQuery = mysql_query("SELECT fournisseurs_company FROM fournisseurs WHERE fournisseurs_id = '".$arrSelect['fabricant_id']."'");
                    ...
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="oui" AND $actRes=="oui") $availability = "1";
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="non" AND $actRes=="non") $availability = "0";
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="oui" AND $actRes=="non") $availability = "0";
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="non" AND $actRes=="oui") $availability = "0";
                    if($arrSelect['products_qt']>0 ) $availability = "1";
                    if($arrSelect['products_download']=="yes") $availability = "1";
                    ...
                    $arr[] = $offerId."\t".$marque."\t".$title."\t".$desc."\t".$prix."\t".$discount."\t".$url."\t".$image."\t".$category."\t".$availability."\t".$delivery.". Garantie: ".$granti."\t".$sf."\t".$poidd."\t".$d3e."\t".$ean;
            }


                    header("Content-Type: application/csv-tab-delimited-table");
                    header("Content-disposition:attachment; filename=shopping_".$country.".txt");
                    echo "Référence interne\tMarque / Fabricant\tNom du produit\tDescription du produit\tPrix\tPrix promo\tURL produit\tURL image\tCatégorie\tDisponibilité\tDescription de la disponibilité + Garantie\tExpedition standard\tPoids\tEcotaxe\tEAN";
