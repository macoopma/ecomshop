...
if(isset($_POST['do']) AND $_POST['do']=="delete" AND $_POST['confirm']==OUI) {
   ...
   $alertMessage.= "</td></tr></table>";
   header("Location: bewaarde_winkelmadjes.php?alertMessage=".$alertMessage);
}
?>
<html>
<head>
...
if(!isset($_GET['display'])) print "<center><a href='bewaarde_winkelmadjes.php?display=expire'>".AFFICHER_EXPIRES."</a>";
if(isset($_GET['display']) AND $_GET['display']=='expire') print "<center><a href='bewaarde_winkelmadjes.php'>".AFFICHER_TOUS."</a>";
print "</tr></td></table>";
...
print "<br>";
	if((isset($_GET['action']) and $_GET['action'] == "delete") and (isset($_GET['confirm']) and $_GET['confirm']=="yes")) {
		...
		print "<script type='text/javascript'>
				<!--
				document.location='bewaarde_winkelmadjes.php';
				//-->
				</script>";
	}
}
...
print "<br>";
	...
	while($myhid = mysql_fetch_array($hids)) {
		...
		if(empty($myhid['users_caddie_client_number'])) {
			...
			$yoyoyo = "<a href='myclient.php?id=".$clientNumb."'>".$clientNumb."</a>";
			$dateExp3 = "--";
			$statut = "<img src='im/noPassed.gif' title='".ACT."'>";
		}
