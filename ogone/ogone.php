...
if($_POST['STATUS'] == "5" OR $_POST['STATUS'] == "9") {


        $queryZ = mysql_query("SELECT * FROM users_orders WHERE users_nic = '".$_POST['orderID']."' AND users_total_to_pay = '".$_POST['amount']."'");
        $queryZNum = mysql_num_rows($queryZ);
        if($queryZNum > 0) {
 
            $hoy = date("Y-m-d H:i:s");
  
            mysql_query("UPDATE users_orders SET users_payed = 'yes', users_confirm = 'yes', pp_statut = 'Ref Ogone: ".$_POST['PAYID']."', users_date_payed = '".$hoy."', users_payment = 'cc' WHERE users_nic = '".$_POST['orderID']."'");
            ...
            // alert client confirmation du paiement BE_NL
                            ...
                            $message = $_store."\r\n".$address_street."\r\n".$address_cp." - ".$address_city."\r\n".$address_country."\r\n";
                             ...
                             $message .= $address_city." on ".$resultZ['users_date_added']."\r\n\r\n";
                             $message .= $adreesName.",\r\n";
                             $message .= "Uw betaling werd uitgevoerd en succesvol ontvangen.\r\n";
                             $message .= "Uw bestelling wordt nu behandeld om verzonden te worden.\r\n";
                             $message .= "Ga naar uw klant interface, en volg uw bestelling op...\r\n";
                             $message .= $_store." dankt u voor uw bestelling.\r\n";
                             $message .= "--------------------------------------------------------------------------------------------------\r\n";
                             $message .= "NIC (bestel nummer): ".$resultZ['users_nic']."\r\n";
                             $message .= "Klant interface URL: ".$urlAdminClient."\r\n";
                             $message .= "Klant nummer: ".$resultZ['users_password']."\r\n";
                             $message .= "E-mail adres: ".$resultZ['users_email']."\r\n";
                             $message .= "U kunt zich ook rechtstreeks aanmelden op http://".$www2.$domaineFull." en ga daarna naar 'Uw account' en meldt u aan met uw account nummer en e-mail adres.\r\n";                             
                             $message .= "--------------------------------------------------------------------------------------------------\r\n";
                             if($affiliateAuto=='oui' AND $queryZAffNum>0 AND isset($affAccount)) {
                             		$queryZAffFindCom = mysql_query("SELECT aff_com  FROM affiliation WHERE aff_customer = '".$resultZ['users_password']."'");
									$resultZAffCom = mysql_fetch_array($queryZAffFindCom);
									$affiliateCom2 = $resultZAffCom['aff_com'];
	                             $message .= "\r\n";
								 $message .= $_store." geeft je ".$affiliateCom2." % commissie op al onze artikelen die klanten kopen via u.\r\n";
								 $message .= "Dit geldt niet voor uw eigen aankopen.\r\n";
	                             $message .= "Je hoeft enkel de onderstaande link te sturen naar uw vrienden en kennissen:\r\n";
	                             $message .= "http://".$www2.$domaineFull."/index.php?eko=".$affiliateNumber."\r\n";
	                             $message .= "U kunt op elk ogenblik uw verdiensten zien via \"Uw account\" op http://".$www2.$domaineFull."\r\n";
	                             $message .= "Dit zijn uw login gegevens:\r\n";
	                             $message .= "Affiliate nummer: ".$affiliateNumber."\r\n";
	                             $message .= "Wachtwoord: ".$affiliatePass."\r\n";
	                             $message .= "\r\n";
                             }
                             $message .= "Indien u nog vragen heeft dan kunt u ons steeds contacteren...\r\n";
            ...
            //-----------------------
                            $to2 = $mailOrder;
                            $from2 = $mailOrder;
                            $subject2 = "Er is een nieuwe bestelling - BETAALD MET OGONE - NIC:".$resultZ['users_nic'];
                            $message2 = "Er is een nieuwe bestelling op http://".$www2.$domaineFull."\r\n";
                            $message2 .= "Datum: ".date("d-m-Y H:i:s")."\r\n";
                            $message2 .= "-------------------------------------------------\r\n";
                            $message2 .= "Naan en voornaam: ".$resultZ['users_firstname']." ".$resultZ['users_lastname']."\r\n";
                            $message2 .= "E-mail: ".$resultZ['users_email']."\r\n";
                            $message2 .= "Land: ".$resultZ['users_country']."\r\n";
                            $message2 .= "-------------------------------------------------\r\n";
                            $message2 .= "Betaalwijze: OGONE\r\n";
                            $message2 .= "-------------------------------------------------\r\n";
                            $message2 .= "Bedrag: ".$resultZ['users_total_to_pay']."\r\n";
                            $message2 .= "NIC nummer: ".$resultZ['users_nic']."\r\n";
                            $message2 .= "Klant nummer: ".$resultZ['users_password']."\r\n";
                            $message2 .= "-------------------------------------------------\r\n";
                            $message2 .= "Bekijk deze bestelling: http://".$www2.$domaineFull."/admin/index.php\r\n";
...
?>
<orderID="<?php print $_POST['orderID'];?>" currency="EUR" amount="<?php print $amountTotal;?>">
