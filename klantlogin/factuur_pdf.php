...
$la_link = "../includes/lang/klantlogin/lang".$_GET['l']."/factuur_pdf.php";
include($la_link);
...
function Header() {
    global $www2,$client_nic,$_GET,$store_name,$store_company,$address_street,$numFact,$dateFact,$address_cp,$address_city,$address_country,$tel,$fax,$domaineFull,$mailOrder,$address_autre,$item,$split;
        // nummer klant
         ...
         if($numFact=="") $numFact="XXXX";
          ...
          if($client['users_date_payed'] == "0000-00-00 00:00:00") {
              $dateFact = dateFr($client['users_date_added'],$_GET['l']);
          }
          else {
               $dateFact = dateFr($client['users_date_payed'],$_GET['l']);
          }
          // andere
          ...
          // betaling
          if($client['users_payment'] == "vi") $payment = A1;
          ...
          if($client['users_payment'] == "co") $payment = A4A;
        ...
        $this->Cell(190,6,remApos($store_name),0,0,'L');                  // bedrijfsnaam
        $this->Ln(5);
        $this->SetFont('Arial','',10);
        
        $this->Cell(95,10,remApos($address_street),0,0,'L');                          // adres
        $this->Cell(95,4,A7.' '.$numFact,0,0,'R');
        ##$this->SetFont('Arial','B',10);
        ##$this->SetTextColor(204,0,0);
        ##$this->Cell(10,4,$numFact,0,0,'R');     // factuur nummer
        ##$this->SetFont('Arial','',10);
        ##$this->SetTextColor(0,0,0);
        $this->Ln(4);
        $this->Cell(95,10,$address_cp.' '.remApos($address_city),0,0,'L');   // poscode
        $this->Cell(95,4,A8.': '.$dateFact,0,0,'R');                      // datum
        $this->Ln(4);
        $this->Cell(95,10,remApos($address_country),0,0,'L');   // postcode
        $this->Ln(5);
        if(!empty($tel)) {
        ...
        $this->Ln(4);
        if(!empty($fax)) {
          $this->Cell(95,10,FAX.' : '.$fax,0,0,'L');         // Fax
          $this->Ln(5);
        }
        $this->Cell(95,10,'Website: http://'.$www2.$domaineFull,0,0,'L');
        $this->Ln(4);
        $this->Cell(95,10,'E-mail: '.$mailOrder,0,0,'L');
        
        if(!empty($address_autre)) {
        $this->Ln(5);
        foreach ($split as $item) {
            $this->Cell(95,10,remApos($item),0,0,'L');         // andere
            $this->Ln(4);
        }
        }
        else {
        ...
        if($client['users_refund']=='yes') {
            $titreR = A5." (".REMBOURSEE.")";
        }
        else {
            $titreR = A5;
        }
        $this->Cell(190,8,$titreR,1,0,'L',1);   // factuur
        ...
        $this->Cell(90,10,remApos($client['users_gender']).'. '.remApos($client['users_lastname']).' '.remApos($client['users_firstname']),0,0,'L'); // gegevens
        
        $this->Ln(4);
            if(!empty($addresss[1])) $this->Cell(90,10,remApos($addresss[1]),0,0,'L'); else $this->Cell(90,10,'',0,0,'L');
            $this->Cell(10,10,' ',0,0,'L');
            if(!empty($client['users_company'])) $this->Cell(90,10,remApos($client['users_company']),0,0,'L'); else $this->Cell(90,10,'',0,0,'L'); // bedrijf
        
        $this->Ln(4);
        $this->Cell(90,10,remApos($addresss[2]),0,0,'L');
        $this->Cell(10,10,' ',0,0,'L');
        $this->Cell(90,10,remApos($client['users_address']),0,0,'L');                 // adres klant
        
            $this->Ln(4);
            if(!empty($addresss[3])) $this->Cell(90,10,remApos($addresss[3],0,0,'L')); else  $this->Cell(90,10,'',0,0,'L');                           // waar
            $this->Cell(10,10,' ',0,0,'L');
        ...
        $this->Cell(10,10,' ',0,0,'L');
        $this->Cell(95,10,remApos($client['users_zip']).' '.remApos($client['users_city']),0,0,'L'); // postcode
        $this->Ln(4);
        $this->Cell(90,10,remApos($addresss[6]),0,0,'L');
        $this->Cell(10,10,' ',0,0,'L');
        $this->Cell(95,10,remApos($client['users_country']),0,0,'L'); // postcode

        $this->Ln(15);
        $this->SetTextColor(204,204,204);
        $this->SetDrawColor(204,204,204);
        $this->SetFont('Arial','B',9);
        $this->Cell(190,7,A100,1,0,'L',0);   // factuur
        $this->Ln(7);
        $y2 = $this->GetY();
        ...
        $this->Ln(4);
        $this->SetFont('Arial','B',9);
        $this->Cell(40,5,$client['users_nic'],0,0,'C');
...
if(abs($client['users_contre_remboursement'])>0) {
    ...
    $pdf->Cell(20,4,$sig4.sprintf("%0.2f",abs($client['users_contre_remboursement'])),0,0,'R',1);
}
$pdf->Ln();
// groot totaal
$pdf->SetFillColor(241,241,241);
