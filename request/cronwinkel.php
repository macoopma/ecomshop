<?php
    ...
    $expQuery = mysql_query("SELECT export_auto_shopping
                               FROM admin");
    $exp = mysql_fetch_array($expQuery);
    $exportShopping = $exp['export_auto_shopping'];
    $cats = explode("||",$exportShopping);
...
function shipping_price($originIso,$_pays,$_poids,$activerPromoLivraison,$totalHtFinal,$livraisonComprise) {
    ...
    $Orderquery = mysql_query("SELECT products_garantie_".$cats[0].", products_ean, products_download, products_deee, products_name_".$cats[0].", products_desc_".$cats[0].", products_price, products_image, products_ref, products_id, categories_id, products_qt, fabricant_id, products_weight
                               FROM products
...
echo "Référence interne\tMarque / Fabricant\tNom du produit\tDescription du produit\tPrix\tPrix promo\tURL produit\tURL image\tCatégorie\tDisponibilité\tDescription de la disponibilité + Garantie\tExpedition standard\tPoids\tEcotaxe\tEAN\r\n";
    ...
    if($OrderqueryNum > 0) {
            while ($arrSelect = mysql_fetch_array($Orderquery)) {
                    ...
                    $granti = strip_tags($arrSelect['products_garantie_'.$cats[0]]);
                    
                    $d3e = $arrSelect['products_deee'];
                    
                    $chaine = $offerId."\t".$marque."\t".$title."\t".$desc."\t".$prix."\t".$discount."\t".$url."\t".$image."\t".$category."\t".$availability."\t".$delivery.". Garantie: ".$granti."\t".$sf."\t".$poidd."\t".$d3e."\t".$ean."\r\n";
