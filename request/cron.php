...
include('../configuratie/configuratie.php');
    ...
    $expQuery = mysql_query("SELECT export_auto_leguide
                               FROM admin");
    $exp = mysql_fetch_array($expQuery);
    $exportLeGuide = $exp['export_auto_leguide'];
    $cats = explode("||",$exportLeGuide);
...
function shipping_price($originIso,$_pays,$_poids,$activerPromoLivraison,$totalHtFinal,$livraisonComprise) {
    ...
    $Orderquery = mysql_query("SELECT products_ean, products_garantie_".$cats[0].", products_download, products_name_".$cats[0].", products_desc_".$cats[0].", products_deee, products_price, products_image, products_ref, products_id, categories_id, products_qt, fabricant_id, products_weight
                               FROM products
    ...
    echo "marque\tcategorie\tidentifiant_unique\ttitre\tdescription\tprix\tprix_barre\tURL_produit\tURL_image\tdevise\tdisponibilite\tfrais_de_port\tdelai_de_livraison\tD3E\tgarantie\tean\r\n";
    ...
    if($OrderqueryNum > 0) {
            ...
            while ($arrSelect = mysql_fetch_array($Orderquery)) {
                    ...
                    $prix_barre = str_replace(".",",",$prix_barre);
                    ...
                    if($activerPromoLivraison == "oui" and $prix>=$livraisonComprise) {
                        ...
                        $prixShipping = sprintf("%0.2f",$arrSelect['products_price']);
                        $shipping = shipping_price("FRA","France",$arrSelect['products_weight'],$activerPromoLivraison,$prixShipping,$livraisonComprise);
                    ...
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="oui" AND $actRes=="oui") $availability = 8;
       
                    if($arrSelect['products_qt']>0 ) $delivery = "3-5";
                    if($arrSelect['products_qt']<=0 AND $displayOutOfStock=="oui" AND $actRes=="oui") $delivery = "7-10";
                    if($arrSelect['products_download']=="yes" ) $delivery = "1 heures";
        
                    $d3e = ($arrSelect['products_deee']>0)? "-2" : 0;
         
          
                    $granti = (!empty($arrSelect['products_garantie_'.$cats[0]]))? strip_tags($arrSelect['products_garantie_'.$cats[0]]) : 0;
           
                    $ean = strip_tags($arrSelect['products_ean']);
                    
            
             
                    $chaine = $marque."\t".$category."\t".$offerId."\t".$title."\t".$desc."\t".$prix."\t".$prix_barre."\t".$url."\t".$image."\t".strtoupper(substr($devise,0,3))."\t".$availability."\t".$sf."\t".$delivery."\t".$d3e."\t".$granti."\t".$ean."\r\n";
                    echo $chaine;
            }
